...
namespace AppApi.Repository
{
    public class ProdutoRepository : DapperService, IProdutoRepository
    {
        ...
        {
            var sql = "SELECT Produto.*, QntItem FROM PedidoProduto INNER JOIN Produto ON PedidoProduto.ProdutoId = Produto.Id WHERE PedidoProduto.PEDIDOID = @PedidoId";
            using var conn = await GetDbConnection();
        ...
        {
            var sql = "INSERT INTO PedidoProduto (PEDIDOID, PRODUTOID, QNTITEM) VALUES (@PedidoId, @ProdutoId, @QntItem)";
            using var conn = await GetDbConnection();
            await conn.ExecuteAsync(sql, new { ProdutoId, PedidoId, QntItem });
        }
